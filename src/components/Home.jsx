...
export default function Home() {
  ...
  const handleSubscription = () => {
    ...
    navigate.navigate('Subscriptions', { register: false }); // Navigate to subscription page
  };
  ...
  const handletopProfiles = async () => {
    // let decoded = await getDecodedToken(); // Get the decoded token to access user data

    try {
      let { data: res } = await topProfilesHomePage(); // Fetch the top profiles from the API
      if (res.data) {
        // if (decoded?.user?.subscriptionType === "REGULAR") {
        //   // Filter out profiles that have the same role as the user
        //   const filteredProfiles = res.data.filter(
        //     (item) => item?.role !== Role // Only include profiles with different roles
        //   );

        //   settopprofiles(filteredProfiles); // Set the filtered profiles
        // } else {
        //   // For non-REGULAR subscriptions, set all profiles without filtering
        //   settopprofiles(res.data);
        // }
        settopprofiles(res.data);
      }
    } catch (err) {
      errorToast(err); // Show an error toast if there's a failure
    }
  };
        ...
        >
          <View style={styles1.modalOverlay}>
            <View style={styles1.modalContainer}>
              <Text style={styles1.modalTitle}>Subscription Required</Text>
              <Text style={styles1.modalMessage}>
                "Buy Subscription and Unlock all the features of Plywood Bazar.com"
              </Text>

              <View style={styles1.buttonContainer}>
                {/* Buttons for Subscription and Skip */}
                ...
                {/* Skip Button */}
                <TouchableOpacity style={styles1.skipButton} onPress={() => setShowModal(false)}>
                  <Text style={styles1.skipButtonText}>Skip for Now</Text>
                </TouchableOpacity>
                  ...
                  renderItem={({ item }) => {
                    return (
                      <TopProfileHomeCard
                        ...
                        image={item.bannerImage && item.bannerImage !== " "
                          ? { uri: generateImageUrl(item.bannerImage) }
                          : require('../../assets/img/logo_1.png')
                        }
                        rating={item.rating}
                        Product={item.productsCount}
                        onPress={() => gototopprofile(item)}
